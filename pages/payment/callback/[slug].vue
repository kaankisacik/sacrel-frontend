...
const { auth3DS } = useIyzicoPayment();
  ...
  try {
    ...
    if (callbackData.value.status === "success") {
      if (callbackData.value.mdStatus !== "1") {
        console.log(
          "Success status but mdStatus is not 1:",
          callbackData.value.mdStatus
        );
      ...
      console.log("Auth3DS Request Body:===============================", authBody);
      const authRes = await auth3DS(authBody);
      ...
      if (authRes.status === "success") {
        ...
        const isValidPayment = validatePaymentParameters(
          authRes,
          callbackData.value
        );
...
onMounted(async () => {
  ...
  if (process.client) {
    ...
    const urlCallbackData = {
      ...
      conversationData:
        route.query.conversationData || urlParams.get("conversationData") || "",
      conversationId:
        route.query.conversationId || urlParams.get("conversationId") || "",
      mdStatus: route.query.mdStatus || urlParams.get("mdStatus") || "",
    };

    // Eğer URL'de veriler varsa bunları kullan
    if (
      urlCallbackData.status ||
      urlCallbackData.paymentId ||
      urlCallbackData.conversationId
    ) {
      ...
      console.log(
        "No URL parameters found, waiting for potential POST data..."
      );
