          ...
          <div class="space-y-6">
            ...
            <div class="space-y-4">
              ...
              <div class="space-y-3">
                ...
                <!-- Size Selection -->
                <div
                  v-if="productSizes.length > 0 && sizeGuideVisible"
                  class="space-y-12"
                >
                  ...
                  <div class="grid grid-cols-4 gap-2">
                    <button
                      ...
                      @mouseover="selectedSize = size.value"
...
const loadSameCategoryProducts = async () => {
  ...
  try {
    ...
    const filteredProducts =
      response.products?.filter((p) => p.id !== product.value?.product?.id) ||
      [];
...
const isFavorite = computed(() => {
  return product.value?.product
    ? favoritesStore.isInFavorites(product.value.product.id)
    : false;
});
...
const toggleFavorite = async () => {
  ...
  try {
    const productData = {
      ...
      image: product.value.product.images?.[0]?.url,
      price: product.value.product.variants?.[0]?.calculated_price?.calculated_amount,
    };
