    ...
    <NuxtLink :to="`/products/${product.handle}`" class="w-full max-w-sm mx-auto overflow-hidden cursor-pointer group border-4 border-black">
        <div class="relative h-48 sm:h-64 md:h-80 lg:h-96 xl:h-[500px]  lg:aspect-3-4  overflow-hidden">
            ...
            <button @click.prevent="toggleFavorite"
                ...
                class="absolute top-3 right-3 w-10 h-10 bg-white bg-opacity-90 rounded-full flex items-center justify-center hover:bg-opacity-100 transition-all duration-300 opacity-0 group-hover:opacity-100 disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <div class="p-4">
                ...
                <div class="flex justify-between items-center mt-3 sm:mt-4 lg:mt-0">
                    ...
                    <div v-if="colors && colors.length > 0" class="flex space-x-1">
                        <div v-for="color in colors.slice(0, 3)" :key="color"
...
const colors = productHelper.extractProductFilters(props.product)['colors'] || [];
const prices = (productHelper.extractProductFilters(props.product)['prices']).map((price: number) => price.toFixed(2)) || [];
...
const isFavorite = computed(() => favoritesStore.isInFavorites(props.product.id));
...
const toggleFavorite = async () => {
    try {
        ...
        const productData = {
            ...
            price: props.product.variants?.[0]?.calculated_price?.calculated_amount || parseFloat(prices[0])
        };

        const wasAdded = await favoritesStore.toggleFavorite(props.product.id, productData);
