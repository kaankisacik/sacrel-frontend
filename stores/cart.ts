...
export const useCartStore = defineStore("cart", {
  ...
  getters: {
    ...
    selectedPaymentSession: (state) => {
      return state.cart?.payment_sessions?.find(
        (session) => session.is_selected
      );
    },
  ...
  actions: {
    ...
    async retrieveCart(cartId?: string) {
      console.log("Retrieving cart with ID:", cartId);
    ...
    async updateCart(data: any) {
      ...
      try {
        ...
        const { cart } = await client.store.cart.update(
          cartId,
          data,
          {},
          headers
    ...
    async listShippingMethods() {
      ...
      try {
        ...
        const response = await client.store.fulfillment.listCartOptions({
          cart_id: cartId,
        });
    ...
    async listPaymentMethods(regionId: string) {
      ...
      try {
        ...
        this.paymentMethods =
          response.payment_providers?.sort((a: any, b: any) => {
    ...
    async initiatePaymentSession(data: any) {
      ...
      try {
        ...
        const response = await client.store.payment.initiatePaymentSession(
          this.cart as any,
          data,
          {},
          headers
